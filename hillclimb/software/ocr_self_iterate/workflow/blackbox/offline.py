...
from hillclimb.core.frontier import FrontierModel, FrontierProvider, get_frontier
...
from hillclimb.software.ocr_self_iterate.workflow.feedback import FeedbackBatch, collect_output_feedback
from hillclimb.software.ocr_self_iterate.workflow.offline_train import DocumentBatch, OfflineTrainConfig
from hillclimb.software.ocr_self_iterate.workflow.registry import WorkflowMetrics, WorkflowRegistry
from hillclimb.software.ocr_self_iterate.workflow.rewards import HybridRewardFramework


@dataclass
class BlackboxOfflineTrainer:
    """
    Offline workflow maturation using black-box frontier models (e.g. Gemini).

    No model weight training — only workflow config + prompts evolve via
    output feedback propagated backward through frontier API calls.
    """

    documents: DocumentBatch
    frontier: FrontierModel = field(default_factory=lambda: get_frontier(FrontierProvider.MOCK))
    train_config: OfflineTrainConfig = field(default_factory=OfflineTrainConfig)
    run_config: BlackboxRunConfig = field(default_factory=BlackboxRunConfig)
    ...
    def train(self, initial: WorkflowConfig | None = None) -> dict[str, Any]:
        ...
        for epoch in range(self.train_config.epochs):
            ...
            config = propagate_via_frontier(config, batch_feedback, self.frontier)
        ...
        return {
            ...
            "frontier_provider": type(self.frontier).__name__,
        }
    ...
    def _forward_pass(
        ...
        for image, truth, fallback in self.documents:
            ...
            feedback_items.extend(collect_output_feedback(result, truth))
    ...
    @classmethod
    def load_stable(cls, registry_path: Path, documents: DocumentBatch, frontier: FrontierModel):
