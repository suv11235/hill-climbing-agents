...
| 2c | **OCR Offline Train** (feedback → stable workflow) | Backward propagation + registry | `python -m hillclimb.software.ocr_self_iterate.workflow.offline_train_run` |
| 2d | **OCR Black-Box** (Gemini, no training) | Frontier API workflow hill climb | `python -m hillclimb.software.ocr_self_iterate.workflow.blackbox.run --provider gemini` |
